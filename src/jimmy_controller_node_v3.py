...
class JimmyController(threading.Thread):
    ...
    def __init__(self):
        ...
        self.right_elbow_roll = rospy.Publisher('/right_elbow_roll/command', Float64, queue_size=10)
        self.right_elbow_pitch = rospy.Publisher('/right_elbow_pitch/command', Float64, queue_size=10)
        ...
        self.left_elbow_roll = rospy.Publisher('/left_elbow_roll/command', Float64, queue_size=10)
        self.left_elbow_pitch = rospy.Publisher('/left_elbow_pitch/command', Float64, queue_size=10)
        ...
        self.left_elbow_roll_relax = rospy.ServiceProxy(
            '/left_elbow_roll/relax', Relax)
        self.left_elbow_pitch_relax = rospy.ServiceProxy(
            '/left_elbow_pitch/relax', Relax)
        ...
        self.right_elbow_roll_relax = rospy.ServiceProxy(
            '/right_elbow_roll/relax', Relax)
        self.right_elbow_pitch_relax = rospy.ServiceProxy(
            '/right_elbow_pitch/relax', Relax)
        ...
        self.left_elbow_roll_enable = rospy.ServiceProxy(
            '/left_elbow_roll/enable', Enable)
        self.left_elbow_pitch_enable = rospy.ServiceProxy(
            '/left_elbow_pitch/enable', Enable)
        ...
        self.right_elbow_roll_enable = rospy.ServiceProxy(
            '/right_elbow_roll/enable', Enable)
        self.right_elbow_pitch_enable = rospy.ServiceProxy(
            '/right_elbow_pitch/enable', Enable)
        ...
        self.joints = {'R_ELBOW_ROLL': self.right_elbow_roll,
            'R_ELBOW_PITCH': self.right_elbow_pitch,
            'R_SHO_PITCH': self.right_sho_pitch,
            'R_SHO_ROLL': self.right_sho_roll,
            'L_ELBOW_ROLL': self.left_elbow_roll,
            'L_ELBOW_PITCH': self.left_elbow_pitch,
            'L_SHO_PITCH': self.left_sho_pitch,
            'L_SHO_ROLL': self.left_sho_roll,
            'HEAD_PAN': self.head_pan_pub,
            'HEAD_TILT': self.head_tilt_pub}

        self.joint_pos = {'R_ELBOW_ROLL': 512,
            'R_ELBOW_PITCH': 512,
            'R_SHO_PITCH': 512,
            'R_SHO_ROLL': 512,
            'L_ELBOW_ROLL': 512,
    ...
    def relax_servos(self):
        ...
        self.left_elbow_roll_relax()
        self.left_elbow_pitch_relax()
        self.left_sho_pitch_relax()
        self.left_sho_roll_relax()
        self.right_elbow_roll_relax()
        self.right_elbow_pitch_relax()
    ...
    def enable_servos(self):
        ...
        self.left_elbow_roll_enable(True)
        self.left_elbow_pitch_enable(True)
        self.left_sho_pitch_enable(True)
        self.left_sho_roll_enable(True)
        self.right_elbow_roll_enable(True)
        self.right_elbow_pitch_enable(True)
    ...
    def joint_callback(self, data):
        ...
        if not self.ready and self.enable_ready:
            ...
            for joint in fubar:
                ...
                if joint_name == 'left_elbow_roll':
                    self.joint_pos['L_ELBOW_ROLL'] = joint_pos
                    self.init_ready[2] = True
                if joint_name == 'left_elbow_pitch':
                    self.joint_pos['L_ELBOW_PITCH'] = joint_pos
                ...
                if joint_name == 'right_elbow_roll':
                    self.joint_pos['R_ELBOW_ROLL'] = joint_pos
                    self.init_ready[6] = True
                if joint_name == 'right_elbow_pitch':
                    self.joint_pos['R_ELBOW_PITCH'] = joint_pos
                ...
                rospy.loginfo(">>>>> %s (%s)" % (joint_name.upper(), joint_pos))
    ...
    def run(self):
        ...
        rospy.wait_for_service('/left_elbow_roll/relax')
        rospy.wait_for_service('/left_elbow_pitch/relax')
        rospy.wait_for_service('/left_sho_pitch/relax')
        rospy.wait_for_service('/left_sho_roll/relax')
        rospy.wait_for_service('/right_elbow_roll/relax')
        rospy.wait_for_service('/right_elbow_pitch/relax')
        ...
        rospy.wait_for_service('/left_elbow_roll/enable')
        rospy.wait_for_service('/left_elbow_pitch/enable')
        rospy.wait_for_service('/left_sho_pitch/enable')
        rospy.wait_for_service('/left_sho_roll/enable')
        rospy.wait_for_service('/right_elbow_roll/enable')
        rospy.wait_for_service('/right_elbow_pitch/enable')
