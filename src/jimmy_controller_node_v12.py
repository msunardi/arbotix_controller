...
from std_msgs.msg import Int16, Float32, Float64, String, Bool
from arbotix_controller.msg import Wavelet, KBParams
...
class JimmyController(threading.Thread):
    ...
    def __init__(self):
        ...
        rospy.Subscriber('/kb_params', KBParams, self.kbparams_callback)

        # Entry to modify slope value
        rospy.Subscriber('/slope', Float32, self.slope_callback)

        # Pick Midi title
        rospy.Subscriber('/midimotion', String, self.midi_callback)

        # Set Wavelet params
        rospy.Subscriber('/wavelet', Wavelet, self.wavelet_callback)
        ...
        self.midi_titles = ['lullaby_of_birdland', 'dancing_queen', 'pink_panther2', 'auld_lang_syne', 'letitbe', 'tetris_theme', 'super_mario_theme']
        self.midi_title = self.midi_titles[0] # default
        self.med_file_path = '%s/src/%s.med' % (self.rospack.get_path('midi_motion'), self.midi_title )
        ...
        self.kb_res = 32
        self.kb_params = {'t':0.0, 'b':0.0, 'c':0.0, 'r': self.kb_res}
        self.use_mrf = True
        self.use_idle = True
        self.plot_data = True
        self.slope_val = 0.7
        ...
        self.head_pan_set_slope_cw = rospy.ServiceProxy('/head_pan_joint/set_compliance_slope_cw', ComplianceSlopeCW)
        self.head_pan_set_slope_ccw = rospy.ServiceProxy('/head_pan_joint/set_compliance_slope_ccw', ComplianceSlopeCCW)

        self.head_tilt_set_slope_cw = rospy.ServiceProxy('/head_tilt_joint/set_compliance_slope_cw', ComplianceSlopeCW)
        self.head_tilt_set_slope_ccw = rospy.ServiceProxy('/head_tilt_joint/set_compliance_slope_ccw', ComplianceSlopeCCW)

        self.left_elbow_set_slope_cw = rospy.ServiceProxy('/left_elbow/set_compliance_slope_cw', ComplianceSlopeCW)
        self.left_elbow_set_slope_ccw = rospy.ServiceProxy('/left_elbow/set_compliance_slope_ccw', ComplianceSlopeCCW)

        self.left_sho_pitch_set_slope_cw = rospy.ServiceProxy('/left_sho_pitch/set_compliance_slope_cw', ComplianceSlopeCW)
        self.left_sho_pitch_set_slope_ccw = rospy.ServiceProxy('/left_sho_pitch/set_compliance_slope_ccw', ComplianceSlopeCCW)

        self.left_sho_roll_set_slope_cw = rospy.ServiceProxy('/left_sho_roll/set_compliance_slope_cw', ComplianceSlopeCW)
        self.left_sho_roll_set_slope_ccw = rospy.ServiceProxy('/left_sho_roll/set_compliance_slope_ccw', ComplianceSlopeCCW)

        self.right_elbow_set_slope_cw = rospy.ServiceProxy('/right_elbow/set_compliance_slope_cw', ComplianceSlopeCW)
        self.right_elbow_set_slope_ccw = rospy.ServiceProxy('/right_elbow/set_compliance_slope_ccw', ComplianceSlopeCCW)

        self.right_sho_pitch_set_slope_cw = rospy.ServiceProxy('/right_sho_pitch/set_compliance_slope_cw', ComplianceSlopeCW)
        self.right_sho_pitch_set_slope_ccw = rospy.ServiceProxy('/right_sho_pitch/set_compliance_slope_ccw', ComplianceSlopeCCW)

        self.right_sho_roll_set_slope_cw = rospy.ServiceProxy('/right_sho_roll/set_compliance_slope_cw', ComplianceSlopeCW)
        self.right_sho_roll_set_slope_ccw = rospy.ServiceProxy('/right_sho_roll/set_compliance_slope_ccw', ComplianceSlopeCCW)
    ...
    def set_speed(self, s='slow'):  # 7 = slow, 20+ = fast
        ...
        try:
            ...
            rospy.logerr("Something went wrong calling speed service:", se)
            pass
    ...
    def set_slopes(self):
        ...
        try:
            ...
            self.head_tilt_set_slope_ccw(slope)
            self.head_tilt_set_slope_cw(slope)
            self.left_elbow_set_slope_ccw(slope)
            self.left_elbow_set_slope_cw(slope)
            self.left_sho_pitch_set_slope_ccw(slope)
            self.left_sho_pitch_set_slope_cw(slope)
            self.left_sho_roll_set_slope_ccw(slope)
            self.left_sho_roll_set_slope_cw(slope)
            self.right_elbow_set_slope_ccw(slope)
            self.right_elbow_set_slope_cw(slope)
            self.right_sho_pitch_set_slope_ccw(slope)
            self.right_sho_pitch_set_slope_cw(slope)
            self.right_sho_roll_set_slope_ccw(slope)
            self.right_sho_roll_set_slope_cw(slope)
        except Exception as se:
   ...
   # CALLBACKS
    ...
    def page_callback(self, page):
        ...
        # page.data = index of pages to execute
        # List of pre-made motion files in WinRME format
        # THESE ARE CURRENTLY HARDCODED IN WEIRD PLACES
        pages = ['/home/mathias/Downloads/WinRME/Chair-Poses.pagelist',
                 '%s/src/pagelists/test-poses.pagelist' % self.rospack.get_path('arbotix_controller'),
                 '%s/src/pagelists/Chair-Poses.pagelist' % self.rospack.get_path('arbotix_controller'),
                 '%s/src/pagelists/marie_curie2_edited4.pagelist' % self.rospack.get_path('arbotix_controller'),
                 '%s/src/pagelists/PositionSequence4.pagelist' % self.rospack.get_path('arbotix_controller'),
                 '%s/PositionSequence.pagelist' % self.rospack.get_path('rebel_ros')]
        ...
        page_to_load = pages[int(page.data)]
        rospy.loginfo("Loading page: {}".format(page_to_load))
        tree = etree.parse(page_to_load)
    ...
    def update_timing_data(self, sequence, midi_timing, mul=1.0):
        ...
        if len(timing_delta) >= l:
            return [t*mul for t in timing_delta[:l]] # Only return timing data as many as sequence data
        else:
            # return sequence['Time']
            return [t*mul for t in timing_delta]
    ...
    def kbparams_callback(self, params):
        ...
        '''
        # t, b, c, r = [float(i) for i in params.data.split(',')]
        self.kb_params = {'t': params.tension, 'b': params.bias, 'c': params.continuity}
        self.kb_res = params.resolution
        rospy.loginfo("[kbparams_callback] New KB params: {} resolution: {}".format(self.kb_params, self.kb_res))

    def slope_callback(self, slope):
        self.slope_val = slope.data
        rospy.loginfo("[slope_callback] New slope: {}".format(self.slope_val))
        try:
            self.set_slopes()
        except Exception as e:
            rospy.logerr("Something went wrong updating slope value. {}".format(e))

    def midi_callback(self, midiinfo):
        '''Choose midi data
           format: <true/false>|<id>
        '''
        info = midiinfo.data.split('|')  # info: ['true/false', 'id']
        if 'false' in info:
            self.use_midi = False
        else:
            self.use_midi = True
        ...
        self.midi_title = self.midi_titles[int(info[1])]
        # midi_titles = ['lullaby_of_birdland', 'dancing_queen', 'pink_panther2', 'auld_lang_syne']
        # self.midi_title = r.choice(['lullaby_of_birdland', 'dancing_queen', 'pink_panther2', 'auld_lang_syne'])
        self.med_file_path = '%s/src/%s.med' % (self.rospack.get_path('midi_motion'), self.midi_title )
        rospy.loginfo('[midi_callback] use midi: {}, file: {}'.format(self.use_midi, self.midi_title))
        # med_file_path = '%s/src/%s.med' % (self.rospack.get_path('midi_motion'), 'lullaby_of_birdland')
        # med_file_path = '%s/src/%s.med' % (self.rospack.get_path('midi_motion'), 'dancing_queen')
        # med_file_path = '%s/src/%s.med' % (self.rospack.get_path('midi_motion'), 'pink_panther')
        # med_file_path = '%s/src/%s.med' % (self.rospack.get_path('midi_motion'), 'pink_panther2')
    ...
    def wavelet_callback(self, data):
        rospy.loginfo("Kernel: {}, levels: {}".format(data.kernel, data.levels))
    ...
    def plot_callback(self, mode):
        plot_data = mode.data
        rospy.loginfo("[plot_callback] change to: {}plot".format("yes, " if mode.data else "not "))
    ...
    def interpolate2(self, fubar, timing, interp, mrf=True):
        ...
        if interp == 'cubic' or interp == 'linear':
            ...
            #     result = self.wavedef(result)
            #     _x = len(result)
            #     x_intpol = np.linspace(0, l-1, num=_x, endpoint=True)

            # if self.plot_data:
            #     y_intpol_mapp = [self.mapp(f) for f in result]
        ...
        if self.use_mrf:
            fx = self.wavedef(fx)
        _x = len(fx)
        xnew = np.linspace(0, l-1, num=_x, endpoint=True)
    ...
    def kbinterp(self, p_x, p_y, res, t=[0.0], c=[0.0], b=[-1.0]):
    ...
    def wavedef(self, data):

        kern = pywt.Wavelet(self.kernel)
        coeffs = pywt.wavedec(data, kern)

        # depth
        k = len(coeffs)
        # coeffs_to_save = k//2
        coeffs_to_save = min(max(int(k*self.levels), 1), k) # at least 1

        # modifier
        # alpha = {0: 1.0, 1: 1.0, 2: 1.0, 5: 1.0}
        ...
        for j in range(k):
            ...
            #     coeffs[j] *= 1.2
            if j > coeffs_to_save:
                coeffs[j] = np.multiply(coeffs[j], 0.0)
            else:
                coeffs[j] *= alpha[j]
...
def main(args):
    rospy.init_node('jimmy_controller_node_with_midi', anonymous=True)
